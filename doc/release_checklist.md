...
Verified locally and in hosted CI through 2026-08-10:

- Dart formatting is clean.
- Current stable Flutter passes full static analysis and all 228 non-Golden
  behavioral tests.
...
- Flutter 3.32.0 with Dart 3.8.0 passes the complete 228-test non-Golden suite,
  static analysis, and repository guards in hosted CI.
...
- The gallery passes static analysis, all three widget tests, and its Web release
...
- Release Web, Windows, Linux, macOS, Android, and unsigned iOS gallery builds
  succeed in hosted CI. Hosting-specific Web configuration is intentionally
...
- `dart pub publish --dry-run` reports zero warnings and an 811 KB compressed
  archive.
- The official pub.dev API returns 404 for `metro_ui` as of 2026-08-10, so
...
The CI workflow provisions missing example platform scaffolds on the runner.
Linux, macOS, Windows, Android, unsigned iOS, and Web release builds all pass
in the hosted environment.

## 1.0 gates

- **Public API stability and documentation — complete locally.** The package
  is prepared for release as `0.1.0`; the compatibility policy, release-candidate API
...
- **Declared SDK range — complete.** Current stable and Flutter 3.32.0 pass
  static analysis, repository guards, and all 228 non-Golden tests. Canonical
  Windows Goldens pass as a separate visual gate.
...
- **Six-platform validation — complete.** Windows, Web, Linux, macOS, Android,
  and unsigned iOS release builds pass in hosted CI.
