...
| Show/hide panel | `showPanel` and `hidePanel`: 364px logical-edge offset over 550ms | flyout panel duration; actual distance follows the Flutter panel width |
...
| Pivot content out | Pivot calls `slideLeftOut`/`slideRightOut`; 350ms with `cubic-bezier(0.3825,0.0025,0.8775,-0.1075)` and keeps opacity until the end step | `MetroMotion.contentExitCurve`; programmatic Pivot changes animate outgoing and incoming pages independently while direct drag remains under pointer control |
