...
| --- | --- |
| Buttons and icon buttons | Pointer and keyboard activation, 0.975 press feedback, reduced motion, disabled semantics, accessible icon names, and theme precedence in `test/controls/metro_button_test.dart`. |
| Tiles and live tiles | Geometry, press tilt, logical badges, reduced motion, disabled naming, keyboard activation, multi-direction and zoom frame transitions, frame semantics, timer cancellation behavior, and stable frame identity in `test/controls/metro_tile_test.dart` and `test/controls/metro_live_tile_test.dart`. |
...
| Focus groups, entrances, and page routes | Sequential/spatial traversal, preserved activation, staggered content entrance, theme capture, logical-direction transitions, and reduced motion in `test/foundation/metro_focus_traversal_test.dart`, `test/foundation/metro_entrance_test.dart`, and `test/navigation/metro_page_route_test.dart`. |
